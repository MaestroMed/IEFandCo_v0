...
import { deletePost, publishNow } from "./actions";
...
export function BlogTable({ rows }: { rows: Row[] }) {
          ...
          {rows.map((r) => (
            <tr key={r.id} className="border-t transition-colors hover:bg-[var(--bg-muted)]" style={{ borderColor: "var(--border)" }}>
              ...
              <td className="px-4 py-3" style={{ color: "var(--text)" }}>
                <Link href={`/admin/blog/${r.id}`} className="hover:text-primary transition-colors font-medium">
                  {r.title}
                </Link>
              ...
              <td className="px-4 py-3 text-right relative">
                ...
                {openMenu === r.id && (
                  <>
                    <div className="fixed inset-0 z-10" onClick={() => { setOpenMenu(null); setConfirmingId(null); }} />
                    <div
                      ...
                      {r.status !== "published" && (
                        <button
                          ...
                          onClick={() => {
                            setOpenMenu(null);
                            start(async () => {
                              const res = await publishNow(r.id);
                              if (res.ok) {
                        ...
                        >
                          <Rocket className="h-3.5 w-3.5" /> Publier maintenant
                        </button>
                      )}
                      {confirmingId === r.id ? (
                        <button
                          ...
                          onClick={() =>
                            start(async () => {
                              const res = await deletePost(r.id);
                              if (res.ok) {
