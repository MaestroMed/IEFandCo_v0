...
export default async function OG({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const subtitle = cmp
    ? `Comparatif technique · ${cmp.rows.length} critères`
    : "Comparatifs experts — IEF & CO";
  const accent = cmp?.accent ?? "196, 133, 92";
  ...
  return new ImageResponse(
    (
      ...
      >
        {/* Living gradient — comparator accent top-right */}
        <div
          style={{
            position: "absolute",
            top: -200,
        ...
        >
          ...
          <span>Comparatif — Décision experte</span>
        </div>
        ...
        >
          <div style={{ display: "flex" }}>{optionA}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
            <span style={{ color: "#7A7A75", fontWeight: 400, fontStyle: "italic" }}>vs</span>
            <span
              style={{
